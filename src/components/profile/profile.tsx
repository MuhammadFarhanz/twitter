...
function Profile({ children, id }: any) {
  ...
  console.log(id, "lolol");
  // Fetch user data based on the provided ID or the ID of the currently authenticated user
  const { data: userProfile } = useGetUserById(id || currentUser?.id);

  // console.log(currentUser, userProfile);
  // Determine the user data to be used based on the presence of the ID prop
